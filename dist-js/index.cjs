...
async function exec(command, args) {
    try {
        return {
            success: true,
            data: await core.invoke(`plugin:polodb|${command}`, args),
        };
    }
    catch (e) {
        try {
    ...
    makeDocuments(...objects) {
        return objects.map((v) => (Object.keys(v).includes("_id")
            ? Object.keys(v._id).includes("$oid")
                ? { _id: v._id["$oid"], ...lodash.omit(v, "_id") }
                : { _id: null, ...lodash.omit(v, "_id") }
            : v));
    }
...
exports.list_databases = list_databases;
